...
from preservicaservice.preservica_s3_bucket import (
    PreservicaBucketAPI, PreservicaS3BucketBuilder
)
...
def mock_preservica_bucketdetails_api_response(jisc_id='jisc'):
    ...
    b_names = [(
        b'bucket_names', _format_siplocations(
            mock_bucketdetails.pop('bucket_names'),
        ),
    )]
    response_data = dict(
        [
...
def mock_preservica_bucket_builder(jisc_id='jisc', environment='test'):
    ...
    def setup_ssm():
        ...
        ssm_client.put_parameter(
            Name='preservica-adaptor-{}-api-decryption-key'.format(
                environment,
            ),
        ...
        ssm_client.put_parameter(
            Name='preservica-adaptor-{}-{}-preservica-user'.format(
                jisc_id, environment,
            ),
            Value='test@user.com',
            Type='SecureString',
        ...
        ssm_client.put_parameter(
            Name='preservica-adaptor-{}-{}-preservica-password'.format(
                jisc_id, environment,
            ),
            Value='test_password',
            Type='SecureString',
    ...
    mocking_managers = [
        ...
        (
            mock.patch,
            [
                'preservicaservice.preservica_s3_bucket.'
                'PreservicaBucketAPI._get_encrypted_bucket_details',
            ],
            {'side_effect': mock_preservica_bucketdetails_api_response(
                jisc_id,
            )},
        ),
    ]
...
def test_get_bucket_details(mock_get):
    bucket_api = PreservicaBucketAPI(
        test_preservica_url, test_preservica_aes_key,
    )
    bucket_details = bucket_api.get_bucket_details(
...
def test_get_bucket():
    bucket_builder = PreservicaS3BucketBuilder(
        test_preservica_url, 'test', 'eu-west-2',
    )
